...
public protocol JSONRPCDecodable: Decodable {
    init(fromJSONRPC decoder: JSONRPCDecoder) throws
}

/// Provide a default implementation which calls through to `Decodable`. This
/// allows `JSONRPCDecodable` to use the `Decodable` implementation generated by the
/// compiler.
public extension JSONRPCDecodable {
    public init(fromJSONRPC decoder: JSONRPCDecoder) throws {
        try self.init(from: decoder)
    }
}

/// The actual JSONRPC decoder class.
...
public extension JSONRPCDecoder {
    static func decode<T: JSONRPCDecodable>(_ type: T.Type, data: [UInt8]) throws -> T {
        return try JSONRPCDecoder(data: data).decode(T.self)
    }
}

/// The error type.
public extension JSONRPCDecoder {
    /// All errors which `JSONRPCDecoder` itself can throw.
    enum Error: Swift.Error {
        ...
        case typeNotConformingToJSONRPCDecodable(Decodable.Type)
...
public extension JSONRPCDecoder {
    ...
    func decode<T: Decodable>(_ type: T.Type) throws -> T {
        ...
        case let intT as FixedWidthInteger.Type:
            return try intT.from(jsonrpcDecoder: self) as! T
        ...
        case let binaryT as JSONRPCDecodable.Type:
            return try binaryT.init(fromJSONRPC: self) as! T
            
        default:
            throw Error.typeNotConformingToJSONRPCDecodable(type)
        }
    }
}

/// Internal methods for decoding raw data.
private extension JSONRPCDecoder {
...
extension JSONRPCDecoder: Decoder {
...
private extension FixedWidthInteger {
    static func from(jsonrpcDecoder: JSONRPCDecoder) throws -> Self {
        var v = Self.init()
        try jsonrpcDecoder.read(into: &v)
