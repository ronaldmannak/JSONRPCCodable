...
/// allows `JSONRPCResultDecodable` to use the `Decodable` implementation generated by the
/// compiler.
public extension JSONRPCResultDecodable {
    public init(fromJSONRPC decoder: JSONRPCResultDecoder) throws {
        try self.init(from: decoder)
    }
}

/// The actual JSONRPC Result decoder class.
public class JSONRPCResultDecoder {
...
public extension JSONRPCResultDecoder {
    static func decode<T: JSONRPCResultDecodable>(_ type: T.Type, data: [UInt8]) throws -> T {
        return try JSONRPCResultDecoder(data: data).decode(T.self)
    }
}

/// The error type.
public extension JSONRPCResultDecoder {
    /// All errors which `JSONRPCResultDecoder` itself can throw.
    enum Error: Swift.Error {
        /// The decoder hit the end of the data while the values it was decoding expected
        /// more.
        case prematureEndOfData
        
        /// Attempted to decode a type which is `Decodable`, but not `JSONRPCDecodable`. (We
        /// require `JSONRPCDecodable` because `JSONRPCDecoder` doesn't support full keyed
        /// coding functionality.)
        case typeNotConformingToJSONRPCResultDecodable(Decodable.Type)
...
public extension JSONRPCResultDecoder {
    ...
    func decode<T: Decodable>(_ type: T.Type) throws -> T {
        ...
        case let intT as FixedWidthInteger.Type:
            return try intT.from(jsonrpcResultDecoder: self) as! T
        ...
        case let binaryT as JSONRPCResultDecodable.Type:
            return try binaryT.init(fromJSONRPC: self) as! T
            
        default:
            throw Error.typeNotConformingToJSONRPCResultDecodable(type)
        }
    }
}

/// Internal methods for decoding raw data.
private extension JSONRPCResultDecoder {
...
extension JSONRPCResultDecoder: Decoder {
    ...
    private struct KeyedContainer<Key: CodingKey>: KeyedDecodingContainerProtocol {
        var decoder: JSONRPCResultDecoder
...
private extension FixedWidthInteger {
    static func from(jsonrpcResultDecoder: JSONRPCResultDecoder) throws -> Self {
        var v = Self.init()
        try jsonrpcResultDecoder.read(into: &v)
